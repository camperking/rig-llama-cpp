...
- [x] **Reconsider re-exporting `llama_cpp_2::context::params::KvCacheType`.** Replaced the upstream re-export with a local `KvCacheType` shim (`#[non_exhaustive]`, mirrors all 31 named upstream variants, `From<KvCacheType> for llama_cpp_2::context::params::KvCacheType` for the FFI conversion). Conversion happens at exactly one site (`checkpoint.rs::ensure_persistent_ctx`). New upstream `ggml_type` values now land as additive `0.1.x` changes here — they no longer force a breaking release. The CHANGELOG caveat about upstream-pinned semver was removed.
