...
- [ ] **Remove `anyhow` from public signatures.** `Client::from_gguf` returns `anyhow::Result<Self>`; `Client::reload` returns `Result<(), String>`. Define a `thiserror` enum (`LoadError { BackendInit, Fit, ModelLoad, MmprojInit, InvalidPath, ... }`) and remove `anyhow` from `[dependencies]` (keep in `dev-dependencies` for examples). `String` errors lose downcastability and force users to string-match.
- [ ] **Add a builder for `Client::from_gguf`.** Already 6 positional params; mmproj makes 7. A `Client::builder().model(p).n_ctx(8192).sampling(...).build()` keeps additions backward-compatible and matches Rig's ergonomics.
