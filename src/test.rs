...
async fn run_streaming_turn(
	...
	ensure!(usage.output_tokens > 0, "streaming turn returned zero output tokens");
...
async fn attempt_tool_call(model: &Model, summary: &mut RunSummary) -> anyhow::Result<()> {
	...
	let follow_up = model
		...
		.preamble("Finish the function-calling validation by using the provided tool result.".to_string())
		.messages(vec![
			Message::user(prompt),
